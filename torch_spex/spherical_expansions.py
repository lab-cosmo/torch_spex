...
class SphericalExpansion(torch.nn.Module):
    ...
    def __init__(
        ...
        self.vector_expansion_calculator = VectorExpansion(
            hypers, self.all_species, device=device, dtype=dtype
        )
    ...
    def forward(
        ...
        if self.is_alchemical:
            ...
            unique_species = -torch.arange(
                self.n_pseudo_species, dtype=torch.int64, device=density_indices.device
            )
        else:
            aj_metadata = samples_metadata.column("species_neighbor")
            aj_shifts = torch.tensor(
                [species_to_index[int(aj_index)] for aj_index in aj_metadata],
                dtype=torch.int64,
                device=aj_metadata.device,
            )
            ...
            for l in range(l_max + 1):
                ...
                densities.append(densities_l)
            unique_species = torch.tensor(
                self.all_species, dtype=torch.int, device=species.device
            )
        ...
        for l in range(l_max + 1):
            ...
            vectors_l_block_n = torch.arange(
                len(torch.unique(vectors_l_block.properties.column("n"))),
                dtype=torch.int64,
                        ...
                        properties=Labels(
                            ...
                            values=torch.stack(
                                [
                                    ...
                                    torch.tile(
                                        vectors_l_block_n, (unique_species.shape[0],)
                                    ),
                                    l
                                    * torch.ones(
                                        (densities_ai_l.shape[2],),
                                        dtype=torch.int,
                                        device=densities_ai_l.device,
                                    ),
                                ],
                                dim=1,
                            ),
...
class VectorExpansion(torch.nn.Module):
    ...
    def forward(
        ...
        r = torch.linalg.norm(bare_cartesian_vectors, dim=-1)
        # r = torch.sqrt(
        ...
        if self.normalize:
            spherical_harmonics *= (4 * torch.pi) ** (0.5)  # normalize them
        ...
        for l, (radial_basis_l, spherical_harmonics_l) in enumerate(
            ...
            if (
                ...
                vector_expansion_l = radial_basis_l.unsqueeze(
                    1
                ) * spherical_harmonics_l.unsqueeze(2).unsqueeze(3)
                n_max_l = vector_expansion_l.shape[3]
            else:
                vector_expansion_l = radial_basis_l.unsqueeze(
                    1
                ) * spherical_harmonics_l.unsqueeze(2)
                        ...
                        [
                            torch.repeat_interleave(
                                -torch.arange(
                                    ...
                                    dtype=torch.int64,
                                    device=vector_expansion_l.device,
                                ),
                                n_max_l,
                            ),
                            torch.tile(
            ...
            vector_expansion_blocks.append(
                TensorBlock(
                    values=vector_expansion_l.reshape(
                        vector_expansion_l.shape[0], 2 * l + 1, -1
                    ),
        ...
        vector_expansion_tmap = TensorMap(
            keys=Labels(
                names=("l",),
                values=torch.arange(start=0, end=l_max + 1, dtype=torch.int32).reshape(
                    l_max + 1, 1
                ),
            ),
