...
class RadialBasis(torch.nn.Module):
    def __init__(
        ...
        if hypers["type"] == "le":
            self.n_max_l, self.spliner = get_le_spliner(
                hypers["E_max"],
                hypers["r_cut"],
                hypers["normalize"],
                device=device,
        ...
        if "alchemical" in hypers:
            ...
            self.combination_matrix = normalize(
                "embedding",
                torch.nn.Linear(
                    len(all_species),
                    self.n_pseudo_species,
                    bias=False,
                    device=device,
                    dtype=dtype,
                ),
        ...
        if hypers["mlp"]:
            ...
            self.all_species_names = (
                list(range(self.n_pseudo_species))
                if "alchemical" in hypers
                else all_species
            )
            self.radial_mlps = torch.nn.ModuleDict(
                {
                    str(l)
                    + "_"
                    + str(aj): torch.nn.Sequential(
                        normalize(
                            "linear_no_bias",
                            torch.nn.Linear(self.n_max_l[l], 32, bias=False),
                        ),
                        normalize("activation", torch.nn.SiLU()),
                        normalize(
                            "linear_no_bias", torch.nn.Linear(32, 32, bias=False)
                        ),
                        normalize("activation", torch.nn.SiLU()),
                        normalize(
                            "linear_no_bias", torch.nn.Linear(32, 32, bias=False)
                        ...
                        normalize(
                            "linear_no_bias",
                            torch.nn.Linear(32, self.n_max_l[l], bias=False),
                        ),
                    )
    ...
    def radial_transform(self, r, samples_metadata: Labels):
        if self.is_physical:
            ...
            x = r / (
                0.1
                + torch.exp(self.lengthscales[a_i])
                + torch.exp(self.lengthscales[a_j])
            )
            return x
        else:
            return r
    ...
    def forward(self, r, samples_metadata: Labels):
        ...
        if self.is_physical:
            radial_functions = torch.where(
                x.unsqueeze(1) < 10.0, self.spliner.compute(x), 0.0
            )
        else:
            radial_functions = self.spliner.compute(x)

        if self.is_alchemical:
            self.species_neighbor_labels.to(
                samples_metadata.values.device
            )  # Move device if needed
            one_hot_aj = custom_one_hot(samples_metadata, self.species_neighbor_labels)
            pseudo_species_weights = self.combination_matrix(
                one_hot_aj.to(
                    device=radial_functions.device, dtype=radial_functions.dtype
                )
            )
            radial_functions = radial_functions.unsqueeze(
                1
            ) * pseudo_species_weights.unsqueeze(2)
        ...
        radial_basis = torch.split(
            radial_functions, self.n_max_l, dim=self.split_dimension
        )

        if self.apply_mlp:
            radial_basis_after_mlp = [
                torch.zeros_like(radial_basis[l]) for l in range(self.l_max + 1)
            ]
            if self.is_alchemical:
                for l_alphaj, radial_mlp_l_alphaj in self.radial_mlps.items():
                    ...
                    radial_basis_after_mlp[l][:, alphaj, :] = radial_mlp_l_alphaj(
                        radial_basis[l][:, alphaj, :]
                    )
            else:
                radial_basis_after_mlp = [
                    torch.zeros_like(radial_basis[l]) for l in range(self.l_max + 1)
                ...
                for l_aj, radial_mlp_l_aj in self.radial_mlps.items():
                    ...
                    radial_basis_after_mlp[l][where_aj, :] = radial_mlp_l_aj(
                        torch.index_select(radial_basis[l], 0, where_aj)
                    )
...
def custom_one_hot(samples_metadata: Labels, species_neighbor_labels: Labels):
    species_mapping = {
        v.item(): k for k, v in enumerate(species_neighbor_labels.values)
    }
    species_neighbors = samples_metadata["species_neighbor"].tolist()
    num_edges = len(species_neighbors)
    num_species = len(species_neighbor_labels)
    species_index_column = torch.arange(num_edges)
    species_index_row = torch.tensor(
        [species_mapping[item] for item in species_neighbors]
    )

    species_neigbor_index = (species_index_column, species_index_row)
    one_hot_aj = torch.zeros((num_edges, num_species))
    one_hot_aj[species_neigbor_index] = 1.0
    return one_hot_aj
